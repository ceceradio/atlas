...
async function buildChoreDefinitionsPrompt(
...
Chore splitting is an important process that breaks down a message describing chores into its individual chore components. By splitting chores accurately, you set up the chore rater to give more precise ratings for each distinct chore, which leads to a more accurate and fair tally of chore performance. Accuracy is important, so it's best to split chores into more items rather than fewer, and to try to capture each bit of the text of the original message. The chore rater can always decide to rate a chore as "not a chore" so there is no harm in splitting chores into more granular items, but if you fail to split out distinct chores, then the chore rater won't be able to rate those distinct chores individually, and that can lead to less accurate ratings and an unfair tally of chore performance.

Chores could be provided in a list format, in a paragraph format, or mixed. Even in a list format, two or more chores could be in a single list item, or a single chore could be split across multiple list items. A message could have 3 bullet points, but there could be 10 or more chores described in those 3 bullet points, and you should try to split them all out into separate chores. On the other hand, if the input is "cooked dinner for 5/5! and helped clean up after dinner (threw out beans; wrapped up and put in the fridge guac, fajita meat, tomates, and lettuce; fridged salsa; put away tortillas)", then the output should be \`["cooked dinner", "took care of dinner leftovers"]\` because the second part is all related to cleaning up after dinner and could be considered one chore, even though it involves multiple steps.
...
Tool call: \`ChoreSplitter({ "chores": ["cleaned the kitchen countertop(s)", "partially cleaned the dryer", "partially cleaned the washer"] })\`
...
Tool call: \`ChoreSplitter({ "chores": ["cleaned the kitchen counters", "cleaned the game room table", "cleaned pink bathroom shower"] })\`
...
Tool call: \`ChoreSplitter({ "chores": ["ordered groceries", "put away groceries", "cleaned the microwave countertop", "cleaned the spice countertop"] })\`
...
Tool call: \`ChoreSplitter({ "chores": ["cleaned the kitchen sink", "cleaned the silverware tray", "reorganized the shelves above the spice counter", "quick cleaned the kitchen counters"] })\`
...
Tool call: \`ChoreSplitter({ "chores": ["replaced air filter", "vacuumed the unit", "partially loaded the dishwasher"] })\`

Input: \`unloaded the dishwasher (morning), unloaded and loaded the dishwasher (evening), took out cans and recycles\`
Tool call: \`ChoreSplitter({ "chores": ["unloaded the dishwasher", "unloaded the dishwasher", "loaded the dishwasher", "took out can recycling", "took out small recycling"] })\`
...
Tool call: \`ChoreSplitter({ "chores": ["swept garage floor", "vacuumed the narrow stairway", "vacuumed the grand stairway", "brought the the recycle bin back inside", "cleaned a slow tub drain"] })\`
...
Tool call: \`ChoreSplitter({ "chores": ["ordered groceries", "put away groceries", "cooked dinner", "took out can recycling", "took out small recycling"] })\`
...
Tool call: \`ChoreSplitter({ "chores": ["loaded the dishwasher", "started the dishwasher", "put away drying rack dishes", "small unloaded dishwasher", "thoroughly cleaned the sink"] })\`
...
Tool call: \`ChoreSplitter({ "chores": ["cooked dinner", "took care of dinner leftovers",  "vacuumed the green bathroom", "wet swiffed the green bathroom", "did the green bathroom towel laundry", "took out can recycling", "took out small recycling", "took out big recycling"] })\`
...
Tool call: \`ChoreSplitter({ "chores": ["cooked dinner", "ordered groceries", "put away groceries", "hand washed a small saucepot"] })\`

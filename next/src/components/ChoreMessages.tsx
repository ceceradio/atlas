...
export function ChoreMessagesPanel() {
  ...
  const [messages, setMessages] = useState<ChoreMessageItem[]>([])
  ...
  }, [token, page, authorId, from, to, noChores])

  useEffect(() => {
    if (token) getAuthors(token).then(setAuthors)
  }, [token])
  ...
  async function handleRescan(id: string) {
    ...
    setRescanningId(id)
    try {
      await reprocessChoreMessage(token, id)
      fetchMessages()
    } finally {

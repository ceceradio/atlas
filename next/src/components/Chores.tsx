...
export function ChoresPanel() {
  ...
  const [authors, setAuthors] = useState<ChoreAuthor[]>([])
  ...
  const fetchChores = useCallback(() => {
    if (!token) return
    setLoading(true)
    getChores(token, { page, limit: LIMIT, discordAuthorId: authorId || undefined, from: from || undefined, to: to || undefined })
      .then((res) => {
        setChores(res.data)
        setTotal(res.total)
      })
      .finally(() => setLoading(false))
  }, [token, page, authorId, from, to])

  useEffect(() => {
    if (!token) return
    getAuthors(token).then(setAuthors)
    getDiscordChannels(token).then(setDiscordChannels)
  ...
  async function handleRescan(choreMessageId: string) {
    ...
    try {
      await reprocessChoreMessage(token, choreMessageId)
      fetchChores()
    } finally {
  ...
  async function saveEdit(id: string) {
    if (!token) return
    await updateChore(token, id, editForm)
    setEditingId(null)
    fetchChores()
  }
